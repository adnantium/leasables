...
contract('TestTimeMachine', function(accounts) {
    ...
    it("Checking time mgmt with LeaseAgreement...", async function() {
        ...
        tx = await test_car1.initiateAgreement(car1_contract.address, tm.address);
        var executor_uid = tx.logs[0].args.agreement_executor;
        const executor = await AgreementExecutorArtifact.at(executor_uid);
    
        time_till_start = await executor.timeTillStart.call(acct_gas);
        ...
        time_till_end = await executor.timeTillEnd.call(acct_gas);
