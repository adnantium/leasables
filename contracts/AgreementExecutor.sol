...
/** @title Lease Agreement Executor
...
contract AgreementExecutor {
    ...
    LeaseAgreement public lease_agreement;
    ...
    /** @dev Creator
      * @param _lease_agreement Address of agreement that executor will manage
      * @param _time_machine FOR DEV ONLY! the time source for 'what time is it now?'
      */    
    constructor (
        address _lease_agreement, 
        ...
        public 
    {
        ...
        // require: check _lease_agreement exists and looks good enough
        lease_agreement = LeaseAgreement(_lease_agreement);

        // TODO: trouble waiting to happen!
        // copying for convienience - probably better to keep in one place though
        // Should be getting these details 'thru' the lease_agreement, not copying on creation
        // Will fail crazy if (when) lease is transferred to another driver or rate changes
        the_car = lease_agreement.the_car();
        the_driver = lease_agreement.the_driver();
        daily_rate = lease_agreement.daily_rate();
        wei_per_sec = lease_agreement.wei_per_sec();
    ...
    function timeTillStart() public view returns (uint time_till_start)
    {
        ...
        if (the_time_now > lease_agreement.start_timestamp()) {
            return 0;
        }        
        return lease_agreement.start_timestamp() - the_time_now;
    }
    ...
    {
        uint the_time_now = time_machine.time_now();
        if (the_time_now > lease_agreement.end_timestamp()) {
            return 0;
        }        
        return lease_agreement.end_timestamp() - the_time_now;
    }
        ...
        driver_only("Only driver can sign agreement as the driver!")
    {
        ...
        require(msg.value >= lease_agreement.driver_deposit_required(), "Insufficient deposit amount!");

        driver_deposit_amount = lease_agreement.driver_deposit_required();
        ...
        if (msg.value > lease_agreement.driver_deposit_required()) {
            driver_balance = msg.value - lease_agreement.driver_deposit_required();
        ...
        owner_only("Only owner can sign agreement as the owner!")
    {
        ...
        require(msg.value >= lease_agreement.owner_deposit_required(), "Insufficient deposit amount!");
